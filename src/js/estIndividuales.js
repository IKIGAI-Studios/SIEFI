...
$('#div-scnd-selects').on('change', '#dateCOPscnd', function() {
    ...
    if ($(this).val() != "false") 
        if ($('#dateRCVscnd').val() != "false" && $('#dateCOINscnd').val() != "false") {
            if ($('#dateRCVscnd').val() != $(this).val() || $('#dateCOINscnd').val() != $(this).val()) bsAlert('Se recomienda utilizar fechas iguales para evitar confusion a la hora de calcular los dias restantes', 'warning')
...
$('#div-scnd-selects').on('change', '#dateRCVscnd', function() {
    ...
    if ($(this).val() != "false") 
        if ($('#dateCOPscnd').val() != "false" && $('#dateCOINscnd').val() != "false") {
            if ($('#dateCOPscnd').val() != $(this).val() || $('#dateCOINscnd').val() != $(this).val()) bsAlert('Se recomienda utilizar fechas iguales para evitar confusion a la hora de calcular los dias restantes', 'warning')
            $('#nombreEjecutor').prop('disabled', false);
        }
});

$('#div-scnd-selects').on('change', '#dateCOINscnd', function() {
    $('#dateCOINfrst option').prop('disabled', false);
    $(`#dateCOINscnd option[value="${$(this).val()}"]`).prop('disabled', $(this).val() != 'false');
    if ($(this).val() != "false") 
        if ($('#dateCOPfrst').val() != "false" && $('#dateRCVfrst').val() != "false") {
            if ($('#dateCOPfrst').val() != $(this).val() || $('#dateRCVfrst').val() != $(this).val()) bsAlert('Se recomienda utilizar fechas iguales para evitar confusion a la hora de calcular los dias restantes', 'warning')
